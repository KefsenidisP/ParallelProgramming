// οι καταναλωτές δεν σταματούν: τερματισμός με CTRL-C

// This is for size = 1.

/*
 * To achieve the producer first then consumer order, each thread locks its 
 * function using the according mutex (mut_del/mut_add) and unlocks the opposite
 * mutex (mut_add/mut_del).
*/

#include <pthread.h>
#include <stdio.h>
#include <stdlib.h>
#include <unistd.h>

#define SIZE 1   // το μέγεθος του κυκλικού buffer
#define LOOP 10   // αριθμός επαναλήψεων παραγωγού
#define NPRO 5   // αριθμός παραγωγών
#define NCON 5    // αριθμός καταναλωτών
#define DPRO 100000  // καθυστέρηση παραγωγού
#define DCON 200000  // καθυστέρηση καταναλωτή

typedef struct {
	int buffer[SIZE];     // ο κυκλικός buffer
	int front, back;      // αριθμοδείκτες προσθήκης και αφαίρεσης 
	int counter;          // μετρητής πλήθους στοιχείων
	pthread_mutex_t *mut_add; // mutex for the add threads
    pthread_mutex_t *mut_del; // mutex for the del threads
	pthread_cond_t *condFull, *condEmpty; // μεταβλητές συνθήκης για συγχρονισμό
} queue;

typedef struct {
	int id;
	queue *fifo;
} thread_data;

// αρχικοποίηση ουράς
queue *queueInit(void) {
	queue *q;

	q = (queue *) malloc(sizeof(queue));
	if (q == NULL)
		return (NULL);

	// αρχικά η ουρά είναι άδεια, αρχικοποιούμε τις μεταβλητές 
	q->counter = 0;
	q->front = 0;
	q->back = SIZE - 1;
	q->mut_add = (pthread_mutex_t *) malloc(sizeof(pthread_mutex_t));
	pthread_mutex_init(q->mut_add, NULL);
    
    // Initialize the lock threads to locked state, as the queue is empty
    q->mut_del = (pthread_mutex_t *) malloc(sizeof(pthread_mutex_t));
    pthread_mutex_init(q->mut_del, NULL);
	pthread_mutex_lock(q->mut_del);

    q->condFull = (pthread_cond_t *) malloc(sizeof(pthread_cond_t));
	pthread_cond_init(q->condFull, NULL);
	q->condEmpty = (pthread_cond_t *) malloc(sizeof(pthread_cond_t));
	pthread_cond_init(q->condEmpty, NULL);

	return (q);
}

// διαγραφή της ουράς
void queueDelete(queue *q) {
	pthread_mutex_destroy(q->mut_add);
    pthread_mutex_destroy(q->mut_del);
	free(q->mut_add);
    free(q->mut_del);
	pthread_cond_destroy(q->condFull);
	free(q->condFull);
	pthread_cond_destroy(q->condEmpty);
	free(q->condEmpty);
	free(q);
}

// προσθήκη στοιχείου στην ουρά
// The add function locks using the mut_add mutex and unlocks the mut_del mutex
void queueAdd(queue *q, int in) {
	// Lock the function from other threads
	pthread_mutex_lock(q->mut_add);
    
    // Removed the while loop wait
	
	// αλιώς προσθέτουμε κανονικά το στοιχείο 
	q->back = (q->back + 1) % SIZE;
	q->buffer[q->back] = in;
	q->counter++;

    puts("Buffer full");

    // Unlock the delete mutex
	pthread_mutex_unlock(q->mut_del);

	return;
}

// αφαίρεση στοιχείου από την ουρά
// Lock the function with mut_del (after it is unlocked by the add function)
// and later unlock the mut_add mutex
void queueDel(queue *q, int *out) {

	// Lock the function from other threads
	pthread_mutex_lock(q->mut_del);

	// αλιώς αφαιρούμε κανονικά το στοιχείο 
	*out = q->buffer[q->front];
	q->front = (q->front + 1) % SIZE;
	q->counter--;

    puts("Buffer empty");

    // Unlock the add function
	pthread_mutex_unlock(q->mut_add);

	return;
}

// το νήμα του παραγωγού (βάζουμε καθυστέρηση για δοκιμή)
void *producer(void *threadarg) {
	int i, myid;
	queue *fifo;
	thread_data *my_data;
	my_data = (thread_data *) threadarg;
	myid = my_data->id;
	fifo = (queue *) my_data->fifo;

	for (i = 0; i < LOOP; i++) {
		printf("Producer %d item %d \n", myid, i);
		queueAdd(fifo, i);
		usleep(DPRO);
	}
	return (NULL);
}

// το νήμα του καταναλωτή (βάζουμε καθυστέρηση για δοκιμή)
void *consumer(void *threadarg) {
	int i, myid;
	queue *fifo;
	thread_data *my_data;
	my_data = (thread_data *) threadarg;
	myid = my_data->id;
	fifo = (queue *) my_data->fifo;

	for (;;) {
		queueDel(fifo, &i);
		printf("Consumer %d item %d \n", myid, i);
		usleep(DCON);
	}
	return (NULL);
}

// η main για να ξεκινήσουμε τα νήματα
int main() {
	queue *fifo;
	pthread_t pro[NPRO], con[NCON];
	thread_data pro_args[NPRO];
	thread_data con_args[NCON];
	int i;

	fifo = queueInit();
	if (fifo == NULL) {
		exit(1);
	}

	for (i = 0; i < NPRO; i++) {
		pro_args[i].id = i;
		pro_args[i].fifo = fifo;
		pthread_create(&pro[i], NULL, producer, &pro_args[i]);
	}
	for (i = 0; i < NCON; i++) {
		con_args[i].id = i;
		con_args[i].fifo = fifo;
		pthread_create(&con[i], NULL, consumer, &con_args[i]);
	}
	for (i = 0; i < NPRO; i++)
		pthread_join(pro[i], NULL);
	for (i = 0; i < NCON; i++)
		pthread_join(con[i], NULL);

	queueDelete(fifo);

	return 0;
}
