...
*
* There is no need to check if the buffer is full, because it will never be.
* The only thing I changed is that I removed the modulo expressions.
*
* The checking for finding out if the buffer is empty is done using the toGet and 
* toPut locks, as I mentioned in the previous code.
*/
...
void queueAdd (queue *q, int in)
{
  ...
  // The modulo expression is not needed
  q->back = (q->back + 1);
  q->buffer[q->back] = in;
