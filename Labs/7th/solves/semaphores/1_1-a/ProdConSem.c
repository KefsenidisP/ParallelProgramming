 ...
 * Code for size = 1
 *
 * Now the size is 0, so only one put-threads at a time will pass the lock statement. 
 * Also, the semaphore is unlocked by the del-thread, whose semaphore (toGet) is initialized to 
 * 0, will proceed inside the get function, only after the put function is called (which unlocks
 * the aforementioned semaphore). The get function later unlocks the toPut semaphore allowing this
 * to continue. Not many changes were made
 */
...
void queueAdd (queue *q, int in)
{
  ...
  // q->back will always be 0, because we have only one element in the buffer
  q->buffer[q->back] = in;
