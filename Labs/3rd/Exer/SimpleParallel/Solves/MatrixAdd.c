...
/*
 * Can be treated like a vector in C.
*/
...
int main(int argc, char* args[])
{
    if (argc != 4)
    {
        printf("Usage: <%s> <rows> <cols> <th_num>", args[0]);
        exit(1);
    }

    th_num = strtol(args[3], NULL, 10);
    
    pthread_t tid[th_num];
    struct tinfo targs[th_num];
    int rows = strtol(args[1], NULL, 10);
    int cols = strtol(args[2], NULL, 10);
    int size = rows * cols;
